...
// HERO 재순서 브리프 — 광선 부팅 안무(bootIn·uBootSpread·개수/속도 램프)를
// 전부 걷어냈다. 위 describe들에서 관련 어서션을 지운 것 자체로는 "되살리면
// FAIL한다"를 보장하지 않는다(지운 테스트는 재도입에 침묵한다) — 소스
// 수준에서 부재를 직접 못박는다. 뮤테이션 (g).
describe('HERO 재순서 — 광선 부팅 안무가 되살아나지 않았다', () => {
  it('Hyperspeed/index.tsx 소스에 bootIn·uBootSpread·광선 램프 식별자가 없다', () => {
    ...
    for (const banned of [
      ...
      'bootSpeedRampActive',
      'bootRampEase',
      'BOOT_LIGHT_RAMP_RATIO',
      'BOOT_LIGHT_RAMP_START_RATIO',
      'BOOT_PEAK_TIME_SCALE',
      'BOOT_START_TIME_FACTOR',
    ]) {
      expect(source, `${banned}가 소스에 남아 있다`).not.toContain(banned);
    }
  });
});
