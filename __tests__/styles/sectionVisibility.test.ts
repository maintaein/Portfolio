...
describe('section visibility utilities', () => {
  ...
  // 상단 여백은 내비게이션 실측 높이 76이다. 내비 높이가 바뀌면 이 숫자도
  // 같이 바뀌어야 한다.
  it('keeps the stage fixed between the navigation and the contact rail', () => {
    const stage = ruleBody('.section-stage');

    expect(stage).toMatch(/position\s*:\s*fixed\s*;/);
    expect(stage).toMatch(
      /inset\s*:\s*76px 0 calc\(45px \+ env\(safe-area-inset-bottom, 0px\)\)\s*;/
    );
