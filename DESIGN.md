...
- 첫 진입 hero. 최초 overview는 검은 배경이고, overview를 처음 떠나는
  순간 배경이 흐림에서 풀리며 오버뷰 밝기로 떠오른다. 모달을 닫고 섹션으로
  돌아올 때와 같은 길이다. 첫 진입 전용 등장 연출은 없다. 밀도가 오르고
  속도는 섹션 전환 boost와 같은 배속까지 빨라졌다가, 섹션 밝기와 섹션 체류
  속도로 가라앉는다. 이 구간에는 전환 boost를 얹지 않는다. 얹으면 상한을
  넘고 가라앉는 구간이 보이지 않는다. 셸과 섹션 진입은 배경이 섹션 밝기에
  닿는 순간까지 `--hero-delay`만큼 기다린다. 워드마크 FLIP만은 기다리지
  않고 배경과 같은 순간에 출발한다. 단계는 `hooks/useHeroPhase.ts`가,
  숫자는 `lib/constants.ts`의 `HERO_*`가 소유한다. 딥링크와 reduced
  motion에서는 재생하지 않고 그 뒤 overview 복귀는 평소와 같다
