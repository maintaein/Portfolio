...
- 해시가 정본이다. 섹션 주소는 `/#projects` 하나뿐이고 `/projects` 같은
  경로는 만들지 않는다. 경로를 하나 더 내면 같은 화면에 주소가 둘이 되고,
  크롤러도 공유 카드도 어느 쪽이 정본인지 모른다. 프로덕션 빌드에 직접
  물어보면 `/projects`는 404이고 `e2e/seo.spec.ts`가 그 404를 지킨다
- `/#<섹션>`으로 바로 들어오면 overview 부팅 안무가 아예 돌지 않는다.
  워드마크는 처음부터 compact이고 START는 없으며 Hyperspeed는 boost 없이
  slow로 시작한다. 루트로 들어오면 그 안무가 처음부터 돈다. 이 짝을
  `e2e/motion.spec.ts`가 세 엔진에서 잰다
- 프로젝트 상세는 주소를 바꾸지 않는다. 열려도 해시는 `#projects` 그대로고
  History 항목 하나만 쌓인다. 그래서 프로젝트마다 공유할 URL이 없고,
  프로젝트별 OG 이미지가 `TODOS.md`에서 연기돼 있다. 재개하려면 이 절의
  해시 정본 계약을 먼저 걷어내야 한다
