...
/* eslint-disable @typescript-eslint/no-explicit-any -- 벤더 사본이다. 원본의
   any를 고치면 렌더 로직에 손을 대는 셈이라 측정값이 원본을 대표하지 못한다.
   Task 4의 정식 포크에서 타입을 정리한다. */

// 임시 계측용 사본 — 계획 3 Task 1 Step 2/3(하강 사다리 실기기 측정) 전용.
// .claude/designRefactoring/Hyperspeed/Hyperspeed.tsx의 사본에 측정 노브 셋을
// 더한 것이다. Task 4의 정식 포크가 아니다. 측정이 끝나면 app/spike와 함께
// 통째로 삭제한다.
//
// 더한 것은 maxDpr / bloomEnabled / bloomResolutionScale 셋뿐이고 원본의 렌더
// 로직은 한 줄도 바꾸지 않았다 — 바꾸면 측정값이 원본을 대표하지 못한다.
...
interface HyperspeedOptions {
  ...
  // --- 계측 노브 (원본에 없음) ---
  maxDpr?: number; // 0이면 기기 네이티브 DPR 그대로
...
class App {
  ...
  constructor(container: HTMLElement, options: HyperspeedOptions) {
    ...
    // 원본은 window.devicePixelRatio를 그대로 썼다. 하강 사다리 4단계를
    // 재려면 상한이 필요해 여기서만 clamp한다.
    const maxDpr = options.maxDpr ?? 0;
    this.renderer.setPixelRatio(
      maxDpr > 0 ? Math.min(window.devicePixelRatio, maxDpr) : window.devicePixelRatio
    );
  ...
  initPasses() {
    ...
    this.bloomPass = new EffectPass(
      ...
      new BloomEffect({
        ...
        resolutionScale: this.options.bloomResolutionScale ?? 1
      })
    );
    ...
    // bloomEnabled=false면 패스를 합성 체인에 넣지 않는다. 객체는 만들되
    // 프레임마다 실행되지 않으므로 p95가 재려는 per-frame 비용은 완전히 빠진다.
    if (this.options.bloomEnabled !== false) {
