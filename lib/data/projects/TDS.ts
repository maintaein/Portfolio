...
export const tds: Project = {
  ...
  reviews: [
    ...
    },
    {
      ...
      problem:
        'recipe()함수 도입 후, vanilla-extract를 쓰는 사용자 환경에서 TDS를 설치하고 빌드하면 Invalid exports 오류가 발생했습니다. 원인은 core의 빌드 산출물(`dist/components/Button/Button.css.mjs`)이 vanilla-extract 플러그인에 의해 재처리되면서, recipe()가 export하는 함수(`createRuntimeFn`)를 만나 실패한 것이었습니다. 라이브러리 제작자 입장에서는 recipe() 함수가 필요했지만, 소비자 입장에서는 빌드가 깨지는 문제였습니다.',
      analysis: [
