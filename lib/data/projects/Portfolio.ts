...
export const portfolio: Project = {
  ...
  implementations: [
    {
      text: 'Atomic Design 아키텍처: 26개 컴포넌트를 3단계로 구조화 (Atoms 13개, Blocks 7개, Sections 6개)',
    },
    {
  ...
  techReasons: [
    ...
    },
    {
      ...
      reasons: [
        ...
        'AnimatePresence로 컴포넌트 언마운트 시 exit 애니메이션 처리, 모달 expand 효과 구현',
    ...
    },
    {
      name: 'Atomic Design',
      reasons: [
        '컴포넌트를 Atoms → Blocks → Sections로 계층화하여 의존 방향을 단방향으로 고정',
  ...
  reviews: [
    {
      ...
      troubleShooting: {
        ...
        initialImpl: 'Modal 컴포넌트를 createPortal로 document.body에 직접 렌더링하도록 구현했습니다. z-index 충돌 없이 최상위에 모달을 올리기 위한 일반적인 방법이었고, 모달이 열릴 때 body scroll을 잠가 배경 스크롤을 막는 처리도 추가했습니다.',
        problem: [
        ...
        analysis: [
          'Hydration mismatch: createPortal은 document.body에 접근하는데, 서버 렌더링 시에는 document가 존재하지 않아 서버에서는 null을 반환하고 클라이언트에서는 Portal을 생성하는 불일치 발생',
          'typeof window !== \'undefined\' 체크를 시도했으나 hydration 에러가 여전히 발생. React의 hydration은 서버 HTML과 클라이언트 첫 렌더를 비교하는데, typeof window 체크는 런타임 분기일 뿐 클라이언트 첫 렌더 결과를 바꾸지 않아 불일치가 그대로 남음',
          'scroll lock 미해제: document.body.style.overflow = "" 로 단순 초기화하면 원래 overflow 값이 ""가 아닌 경우(예: 다른 스타일이 인라인으로 적용된 경우) 복원이 아닌 덮어쓰기가 됨. 또한 isOpen이 false로 바뀔 때 cleanup이 실행되지 않아 모달을 닫아도 lock이 유지되는 케이스 발생',
        ],
        solution: [
          'mounted state 패턴으로 Hydration 해결 — useEffect로 마운트 이후에만 mounted=true 로 전환. useEffect는 서버에서 실행되지 않으므로 서버/클라이언트 첫 렌더 모두 null을 반환해 불일치가 없어짐.',
          'body scroll 복원: overflow 변경 전 기존 값을 originalStyle 변수에 저장하고, useEffect cleanup에서 해당 값으로 복원. isOpen이 false로 바뀌면 dependency 변경으로 cleanup이 자동 실행되어 scroll lock이 해제됨.',
        ],
  ...
  return createPortal(
    ...
    document.body
        ...
        results: [
          'Hydration 에러 해결: 서버와 클라이언트 첫 렌더 모두 null을 반환하여 불일치 없음',
        ...
        lessons: [
          'typeof window 체크는 SSR hydration 에러를 막지 못한다. React hydration은 서버 HTML과 클라이언트 첫 렌더를 비교하므로, 클라이언트 첫 렌더도 서버와 동일한 결과를 반환해야 한다. mounted state + useEffect 패턴이 이를 보장하는 올바른 방법이다.',
          'body.style.overflow = "" 로 단순 초기화하지 말고 열기 전 값을 저장해 복원해야 한다. 원래 값이 ""가 아닌 경우 의도치 않게 덮어쓰게 된다.',
          'useEffect의 cleanup 함수는 컴포넌트 언마운트뿐 아니라 dependency 변경 시에도 실행된다. isOpen=false로 전환될 때 cleanup이 자동 실행되므로, scroll 복원 로직을 별도로 onClose에 넣을 필요가 없다.',
        ]
      }
    },
    {
      ...
      intent: '포트폴리오 전체에서 반복되는 UI 패턴과 로직을 재사용 가능한 컴포넌트와 커스텀 훅으로 추상화하여 코드 중복을 최소화하고 유지보수성을 극대화하고자 했습니다. 특히 AboutSection의 3가지 애니메이션(TechParticleStorm, EmpathyRadar, CollaborationMesh) 구현 시, 각 컴포넌트가 독립적으로 스크롤 감지를 해야 했는데, 매번 IntersectionObserver를 직접 구현하면 코드 중복뿐만 아니라 freezeOnceVisible 로직 구현과 SSR 안전성 처리를 세 번 반복해야 했습니다. 이를 hook으로 추출하는 과정에서 단순히 코드를 줄이는 것을 넘어, Observer cleanup 타이밍과 frozen 상태 처리 방식을 올바르게 설계하는 것이 핵심 과제였습니다.',
      troubleShooting: {
        title: 'freezeOnceVisible 구현 — 애니메이션이 스크롤 아웃 후 재진입 시 재실행되는 문제',
        initialImpl: 'AboutSection의 3가지 애니메이션 컴포넌트에 각각 IntersectionObserver를 직접 구현했습니다. 각 컴포넌트에서 useRef로 DOM을 참조하고, useEffect 안에서 Observer를 생성·등록·cleanup하는 구조였습니다.',
        problem: [
          '사용자가 애니메이션 섹션을 지나쳤다가 다시 스크롤해 올라오면 애니메이션이 처음부터 재실행됨 — 포트폴리오 특성상 한 번만 실행되어야 자연스러운 인상을 줌',
          'hasIntersected 상태를 useState로 관리하면 true로 전환 시 리렌더링이 발생하고, useEffect dependency에 포함되어 Observer가 재등록되는 불필요한 사이클 발생',
          '각 컴포넌트마다 동일한 Observer 초기화·cleanup·freeze 로직(약 30줄)이 반복되어, threshold 조정 같은 사소한 변경도 모든 파일을 수정해야 함'
        ],
        analysis: [
          'freezeOnceVisible의 핵심은 "한 번 isIntersecting=true가 되면 이후 Observer 콜백을 실행할 필요가 없다"는 것. 단순히 isIntersecting 값을 고정하는 것만으로는 부족하고, Observer 자체를 해제해야 불필요한 콜백 호출이 없어짐.',
          '초기 구현에서 hasIntersected를 useState로 관리했을 때의 문제: isIntersecting=true → hasIntersected=true → 리렌더링 → useEffect 재실행 → Observer 재등록 사이클. 이 사이클은 freeze가 된 이후에도 threshold 등 다른 dependency 변경 시마다 반복될 수 있음.',
          'frozen = freezeOnceVisible && isIntersecting 을 useEffect dependency에 포함시키면, frozen이 true가 되는 순간 useEffect가 재실행되고 "if (!element || frozen) return"이 즉시 반환하여 Observer가 재등록되지 않음. 이전 useEffect의 cleanup으로 기존 Observer가 disconnect되므로, frozen 상태에서는 Observer가 아예 존재하지 않게 됨.'
        ],
        solution: [
          'frozen = freezeOnceVisible && isIntersecting 을 useEffect dependency에 추가. frozen이 true가 되는 순간 useEffect가 재실행되고, 함수 최상단에서 frozen 체크로 즉시 반환하여 Observer가 재등록되지 않음. 이전 useEffect cleanup이 기존 Observer를 disconnect하므로 freeze 이후에는 Observer가 완전히 제거됨.',
          ...
          'SSR 안전성은 별도 처리 없이 자동 확보: IntersectionObserver 생성을 useEffect 내부에서만 수행하므로, 서버에서는 useEffect가 실행되지 않아 document 접근 에러가 발생하지 않음.',
        ],
...
export default function EmpathyRadar() {
  ...
  // ...
        ...
        results: [
          'Observer 재실행 완전 차단: frozen 이후 Observer가 disconnect되어 콜백이 호출되지 않고, 리렌더링도 발생하지 않음',
          '코드 중복 제거: 각 컴포넌트의 30줄 Observer 로직이 useIntersection 호출 3줄로 대체, threshold 조정 같은 변경은 hook 옵션으로만 처리',
          'SSR 에러 없음: useEffect 내부에서만 IntersectionObserver를 생성하므로 서버 렌더링 시 별도 guard 불필요'
        ],
        lessons: [
          'freezeOnceVisible 구현에서 핵심은 "상태를 고정"하는 것이 아니라 "Observer를 제거"하는 것이다. frozen을 useEffect dependency에 넣으면 freeze 시점에 cleanup이 자동 실행되어 Observer가 사라지고, 이후 어떤 스크롤 이벤트가 발생해도 콜백이 호출되지 않는다.',
          'useState로 Observer 관련 상태를 관리하면 값이 바뀔 때마다 리렌더링 → useEffect 재실행 → Observer 재등록 사이클이 발생한다. 리렌더링이 불필요한 좌표·플래그 값은 useRef로 관리하고, UI에 반영해야 하는 isIntersecting 같은 값만 useState로 관리하는 것이 맞다.',
          '제네릭 타입 파라미터는 hook의 유연성과 타입 안전성을 동시에 확보한다. useIntersection<HTMLSectionElement>()처럼 호출부에서 타입을 지정하면 잘못된 요소 타입에 ref를 붙이는 실수를 컴파일 타임에 잡을 수 있다.'
        ]
      }
    },
    {
    ...
    },
    {
      ...
      title: '4. 성능 최적화',
      features: [
        '동적 임포트: Modal 컴포넌트를 next/dynamic으로 초기 번들에서 분리, 필요 시점에 로드',
      ...
      intent: '사용자에게 빠른 초기 로딩과 부드러운 스크롤 인터랙션을 제공하기 위해 번들 분리, 스크롤 이벤트 최적화, 이미지/폰트 최적화를 적용했습니다. 특히 스크롤 이벤트는 매 프레임마다 수십 번 발생하므로 requestAnimationFrame과 커스텀 throttle interval을 조합해 불필요한 상태 업데이트를 줄였습니다.'
    }
  ],
